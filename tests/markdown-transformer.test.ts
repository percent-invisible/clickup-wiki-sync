...
describe('MarkdownTransformer', () => {
    ...
    it('rewrites ClickUp doc links to local index.md', async () => {
    ...
    it('rewrites ClickUp page links to mapped local path', async () => {
    ...
    it('leaves unchanged local links that are already relative', async () => {
    ...
    it('leaves unknown links unchanged', async () => {
    ...
    it('replaces empty link text with mapped page name for ClickUp page links', async () => {
    ...
    it('generates correct relative paths for files in different folders', async () => {
        ...
        const resultFromDeep = await transformer.transform({
        ...
        const resultToDeep = await transformer.transform({
        ...
        const expectedLinkSameFolder = path.relative(
            path.dirname(currentFilePathSameFolder),
            pageMapping['jkl012'].path,
        );
        const expectedSameFolderLink = expectedLinkSameFolder.startsWith('.')
            ? expectedLinkSameFolder
            : './' + expectedLinkSameFolder;
        const resultSameFolder = await transformer.transform({
